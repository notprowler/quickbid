...
const rejectAllBids: (itemID: string, bidID: number) => Promise<void> = async (
  itemID: string,
  bidID: number
) => {
  const { data, error } = await supabase
...
// PRE: Check if new bidder is same as old bidder - Dont Give back the old Bidders money back to his account
// PRE: if a user places another bid conserqutively. only deduct the difference from old bid from wallet.
...
const placeBid: RequestHandler = async (req: Request, res: Response) => {
  ...
  if (!itemID || !bidderID || !bidValue) {
    res.status(400).json({
      error: "Item ID, Bidder ID, Bid Value, and Bid Deadline are required",
    });
    return;
  }
  ...
  try {
    ...
    // ------------------------------------------------------------------------------------------------
    //PRE:
    // if oldbidder == new bidder take out only the difference from his account
    if (previousBidData && previousBidData.bidder_id === bidderID) {
      const difference = bidValue - previousBidData.bid_amount;

      if (difference > 0) {
        const { error: updateError } = await supabase
          .from("users")
          .update({ balance: userData.balance - difference })
          .eq("user_id", bidderID);
